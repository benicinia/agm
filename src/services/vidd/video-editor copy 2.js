...
handleVideoEnded() {
    console.log('Video ended, looking for next content...');
    
    // Find the video clip that just ended
    const endedVideoClip = this.clips.find(clip => 
        clip.id === this.currentVideoClip && 
        clip.type === 'video'
    );
    
    if (!endedVideoClip) return;
    
    const videoEndTime = endedVideoClip.startTime + endedVideoClip.duration;
    
    // Find the next clip (could be video, image, or text)
    const allClips = [...this.clips].sort((a, b) => a.startTime - b.startTime);
    const nextClip = allClips.find(clip => 
        clip.startTime > endedVideoClip.startTime && 
        clip.startTime >= videoEndTime - 0.1 // Allow small overlap tolerance
    );
    
    if (nextClip) {
        console.log(`Found next clip: ${nextClip.type} at ${nextClip.startTime}s`);
        
        // If next clip is video, let the video player handle it
        // If next clip is image/text, jump to it
        if (nextClip.type !== 'video') {
            this.jumpToTime(nextClip.startTime);
        }
        // If next clip is video, the mainVideoPlayer will handle it via its own ended event
    } else {
        // No more clips, stay at end
        console.log('No more clips after video');
...
async addMediaToLibrary(file) {
 ...
 updateClipElement(clip, element = clip.element) {
    if (!element) return;
        ...
        if (this.draggingHandle) {
            if (this.draggingHandle === 'left') {
                ...
                if (newWidth >= 40) {
                    ...
                    const canResize = this.canResizeClip(clip, newStartTime, newDuration);
                    if (canResize) {
                ...
                if (newWidth >= 40) {
                    ...
                    const canResize = this.canResizeClip(clip, clip.startTime, newDuration);
                    if (canResize) {
                        clip.duration = newDuration;
...
jumpToTime(time) {
    ...
    if (activeVideoClip) {
        ...
        if (this.currentVideoClip !== activeVideoClip.id) {
            // Different video, perform transition
            const previousClip = this.clips.find(c => c.id === this.previousVideoClipId);
            this.currentVideoClip = activeVideoClip.id;
            this.performVideoTransition(previousClip, activeVideoClip, timeInClip);
        } else {
...
animate() {
    ...
    if (this.isPlaying) {
        // Get the current active video clip
        const activeVideoClip = this.findActiveVideoClip(this.currentTime);
        
        // CRITICAL FIX: Only advance timeline manually if NO video is playing
        const isVideoPlaying = activeVideoClip && 
                              this.mainVideoPlayer && 
                              !this.mainVideoPlayer.paused && 
                              !this.mainVideoPlayer.ended;
        
        if (!isVideoPlaying) {
            // No video is playing, advance timeline normally
            ...
            if (this.currentTime >= this.timelineDuration) {
                ...
                // CRITICAL: Stop any video that might be playing
                if (this.mainVideoPlayer && !this.mainVideoPlayer.paused && !this.mainVideoPlayer.stoped) {
                    this.mainVideoPlayer.pause();
                }
            }
        ...
        // If video IS playing, DO NOT advance timeline here
        // The video's timeupdate event will update the timeline
...
updatePreview() {
    ...
    if (activeVideoClip) {
        ...
        // Validate time is within clip bounds
        if (timeInClip < 0) {
            this.currentTime = activeVideoClip.startTime;
            timeInClip = 0;
        } else if (timeInClip > activeVideoClip.duration) {
            // We're past this video clip - find next content
            this.handlePastVideoClip(activeVideoClip);
            return;
        }
        ...
        // Always update overlays (text/images over video)
        this.updateOverlayContent();
        
    } else {
        // No active video at this time
      //  this.handleNoActiveVideo();
    }
...
seekVideoToTime(timeInClip) {
    // Prevent seeking to negative time or beyond duration
    timeInClip = Math.max(0, Math.min(timeInClip, this.getCurrentVideoClip()?.duration || 0));
