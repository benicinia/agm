     ...
     //   librarySection.id = 'librarySection';
        ...
        annotationSection.innerHTML = `  
                <div class="class-input">
                    <input type="text" id="className" placeholder="Enter object class">
                ...
                <button id="uploadImagesBtn">Upload Images</button>
                            <button id="selectModeBtn" class="tool-btn active">Select</button>
                            <button id="drawModeBtn" class="tool-btn">Draw Box</button>
                            <button id="deleteModeBtn" class="tool-btn">Delete</button>
                            <button id="cropModeBtn" class="tool-btn">Crop</button>
                        </div>
                        
                        <div class="bbox-list" id="bboxList">
                            <p>No bounding boxes added yet</p>
                        </div>
        ...
        if (controlPanel.firstChild) {
           // controlPanel.insertBefore(converterSection, controlPanel.firstChild);
            controlPanel.insertBefore(annotationSection, controlPanel.firstChild);
        } else {
            controlPanel.appendChild(annotationSection);
           // controlPanel.appendChild(converterSection);
        }
        ...
        if (tabs) {
         ...
         //   libraryTab.className = 'tab-btn';
          ...
          //  libraryTab.innerHTML = `
            ...
            annotationTab.dataset.tab = 'annotations';
            annotationTab.innerHTML = ` <h3><i class="fas fa-code"></i> Annotate</h3>`;
           
            tabs.appendChild(annotationTab);
            // Add library tab content
            const tabContents = document.querySelectorAll('.tab-content');
            const lastTabContentv = tabContents[tabContents.length - 3];
            
           
         const similarity= document.getElementById('similarity')
             // Add annotations tab content
           // const tabContents = document.querySelectorAll('.tab-content');
           // const lastTabContent = tabContents[tabContents.length - 1];
            ...
            annotationTabContent.innerHTML = ` <div class="tab-header">
                        <h3><i class="fas fa-code"></i> Annotate</h3>
                        <div class="tab-actions">
                            <input type="text" id="hashSearch" placeholder="Search images or hashes...">
                            <button id="copyHashesBtn" class="btn-small">
                ...
                <button id="uploadImagesBtn">Upload Images</button>
                        ...
                        <div class="instructions">
                            ...
                            <p>- Select a class, then draw boxes</p>
                            <p>- Drag boxes to reposition</p>
                            <p>- Use handles to resize</p>
                            <p>- Double-tap on mobile to select</p>
                        </div>
                    </div>
                    
                    <div class="image-container" id="imageContainer">
                       <img id="currentImage" style="max-width: -webkit-fill-available; display: none;">
                       
                        <canvas id="annotationCanvas">
  </canvas>
                        <!-- Bounding box overlays will be added here -->
                         
                        <div style="position: absolute;z-index: 900;top: 6em;left: 0;" class="image-nav">
                            <button id="prevImageBtn">Previous</button>
                   
                        </div>
                        <div style="position: absolute;z-index: 900;top: 6em;right: 0;" class="image-nav">
                           
                            <button id="nextImageBtn" >Next</button>
                        </div>
                         <div class="box-controls" id="boxControls" style="display: none;position: absolute; top:0; margin-inline: 1em; align-items: flex-end; ">
                           
                            <div class="control-row" style="flex-direction: column;">
                              
                                <select id="boxClassSelect"></select>
                            </div>
                            <div class="control-row">
                                <label>X:</label>
                                <input style="background-color: transparent;" type="number" id="boxXInput" min="0">
                            </div>
                            <div class="control-row">
                                <label>Y:</label>
                                <input type="number" style="background-color: transparent;" id="boxYInput" min="0">
                            </div>
                            <div class="control-row">
                                <label>Width:</label>
                                <input type="number" id="boxWidthInput" min="1">
                            </div>
                            <div class="control-row">
                                <label>Height:</label>
                                <input style="background-color: transparent;" type="number" id="boxHeightInput" min="1">
                            </div>
                            <button id="updateBoxBtn">Update Box</button>
                        </div> 
                    </div>
                </div>
            ...
            if (lastTabContentv) {
               // lastTabContent.parentNode.insertBefore(libraryTabContent, lastTabContent.nextSibling);
                lastTabContentv.parentNode.insertBefore(annotationTabContent, lastTabContentv.nextSibling);
        ...
        setTimeout(() => {
            ...
            document.querySelectorAll('.tab-btn').forEach(btn => {
                if (btn.dataset.tab === 'library') {
                 ...
                 if (btn.dataset.tab === 'annotations') {
                    btn.addEventListener('click', (e) => {
                        document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
                        document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
                        ...
                        //this.displayLibraryGrid(this.getFilteredImages());
                       // this.annotationGrid(this.getAnnotationTools());
                    });
                }
                if (btn.dataset.tab === 'converter') {
                    btn.addEventListener('click', (e) => {
